...
st.markdown("# 🌦️🌦️ ECO SYSTEM CALL 🌦️🌦️")
...
# MENU LATERAL
st.sidebar.header("Selecione os dados do gráfico") #título header e sub subheader
...
if filtros("poeira"):
    def_selecionado = def_selecionado[
        ...
        (def_selecionado["poeira"] <= poeira_range[1])
    ]
...
#Calculos estatísticos
    if not def_selecionado.empty:
        ...
        media_altitude = def_selecionado["altitude"].mean()
        ...
        with media3:
            st.info("Média de registros de altitude", icon="📌")
            st.metric(label="Média", value=f"{media_altitude:.2f}")
...
def graficos():
    ...
    aba1, aba2, aba3,  = st.tabs(["Gráfico de Barras", "Gráfico de Barras","Gráfico de Dispersão"])
...
st.markdown("# FIQUE POR DENTRO! ")
st_player('https://www.youtube.com/watch?v=o7TQx2R24lI')